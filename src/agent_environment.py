...
from cities_and_routes import get_randomly_spread_cities, get_routes
...
if __name__ == "__main__":
    ...
    start_city = 0
    end_city = 9
    player_sprite_path = "assets/lego.png"
    reaper_sprite_path = "assets/reaper.png"
    sprite_speed = 1
    ...
    player_sprite = Sprite(player_sprite_path, cities[start_city])

    reaper_city = random.randrange(1, 9)
    reaper_sprite = Sprite(reaper_sprite_path, cities[reaper_city])
    ...
    while True:
        ...
        if 0 <= int(chr(action)) <= 9:
            if int(chr(action)) != state.current_city and not state.travelling:
                ...
                route_search_1 = [cities[state.current_city], cities[state.destination_city]]
                route_search_1 = tuple(route_search_1)

                route_search_2 = cities[state.destination_city], cities[state.current_city]
                route_search_2 = tuple(route_search_2)

                if (route_search_1 in routes) or (route_search_2 in routes):
        ...
        if state.travelling:
            "reaper will travel at the same time lego does"
        ...
        if not state.travelling:
            encounter_event = False
            "add while loop until path between two cities exists"
        ...
        if state.encounter_event:
            ...
            reaper_sprite.draw_sprite(screen)
        pygame.display.update()
        if state.current_city == end_city:
            print('You have reached the end of the game!')
            break

        if state.reaper_city == state.current_city:
            print('The reaper caught you')
            break
